...
```
cargo install --path .   # installs `monitor` to ~/.cargo/bin
monitor                  # q/esc to quit
```

Sessions come from `~/.claude/projects/*/*.jsonl` (touched in the last 30 min), parsed by an embedded `python3` snippet every 15s: context tokens from the last reply's usage, the label from the first user message. AI processes (highlighted in the network table) are matched by name/exe against `AI_HINTS` in `src/main.rs`. Network rates are macOS-only. Project sizes come from `du -sk` over `~/Documents/Lhava` in a background thread (a directory counts as a project if it holds `.git`, `context/`, `docs/`, or `CLAUDE.md`; descent stops there). Cold scan ~2.5 min, refresh every 30 min.
